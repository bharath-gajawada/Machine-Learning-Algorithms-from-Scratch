...
class MultiLabelMLPClassifier:
    ...
    def forward_prop(self, X):
        ...
        AZ.append((A, Z))
        A = self.activation_function('sigmoid', Z)  # Use sigmoid for multi-label output
        return A, AZ
    ...
    def binary_cross_entropy(self, y, A_final):
        return -np.sum(y * np.log(A_final + 1e-8) + (1 - y) * np.log(1 - A_final + 1e-8)) / y.shape[1]
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.epochs):
            ...
            if self.early_stopping:
                ...
                loss = self.binary_cross_entropy(y, A_final)
        ...
        for i in range(self.hidden_layers + 1):
            for j in range(self.weights[i].shape[0]):
                for k in range(self.weights[i].shape[1]):
                    ...
                    A_final, _ = self.forward_prop(X)
                    loss1 = self.binary_cross_entropy(y, A_final)

                    self.weights[i][j, k] -= 2 * epsilon
                    A_final, _ = self.forward_prop(X)
                    loss2 = self.binary_cross_entropy(y, A_final)
    ...
    def evaluate(self, X, y_true, threshold=0.5):
        ...
        avg_precision = np.mean(precision) if np.sum(tp + fp) > 0 else 0
        avg_recall = np.mean(recall) if np.sum(tp + fn) > 0 else 0
