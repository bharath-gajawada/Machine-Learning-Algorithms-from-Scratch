...
class MLP:
    def __init__(self, type, learning_rate, activation, optimizer, hidden_layers, neurons_per_layer, batch_size, epochs, early_stopping, patience):
        self.type = type
    ...
    def forward_prop(self, X):
        ...
        if self.type == 'classification':
            ...
            A = self.activation_function('linear', Z)
        return A, AZ
        ...
        for i in range(self.hidden_layers + 1):
            for j in range(self.weights[i].shape[0]):
                for k in range(self.weights[i].shape[1]):
                    ...
                    A_final, _ = self.forward_prop(X)
                    loss1 = self.cost(y, A_final)

                    self.weights[i][j, k] -= 2 * epsilon
                    A_final, _ = self.forward_prop(X)
                    loss2 = self.cost(y, A_final)
