...
class MLPRegressor:
    def __init__(self, learning_rate, activation, optimizer, hidden_layers, neurons_per_layer, batch_size, epochs, early_stopping, patience, loss_function="mse"):
    ...
    def forward_prop(self, X):
        ...
        A = self.activation_function('sigmoid', Z)
        return A, AZ
    ...
    def loss(self, y, A_final):
        if self.loss_function == "mse":
            return np.mean(np.square(y - A_final))
        elif self.loss_function == "bce":
            epsilon = 1e-8
            return -np.mean(y * np.log(A_final + epsilon) + (1 - y) * np.log(1 - A_final + epsilon))
        ...
        for i in range(self.hidden_layers + 1):
            for j in range(self.weights[i].shape[0]):
                for k in range(self.weights[i].shape[1]):
                    ...
                    A_final, _ = self.forward_prop(X)
                    loss1 = self.loss(y, A_final)

                    self.weights[i][j, k] -= 2 * epsilon
                    A_final, _ = self.forward_prop(X)
                    loss2 = self.loss(y, A_final)
